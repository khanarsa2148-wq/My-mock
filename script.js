...
async function validateUser() {
    ...
    try {
        ...
        if (result.auth) startExam();
        else {
            showModal("Not Eligible", "Email not authorized.", "🚫");
            btn.disabled = false; btn.innerText = "START EXAMINATION";
        }
    } catch (e) {
        showModal("Error", "Check API URL or Connection");
        btn.disabled = false; btn.innerText = "START EXAMINATION";
    }
}
...
async function endExam() {
    ...
    await fetch(API_URL, {
        method: 'POST',
        body: JSON.stringify({ action: 'submit', studentData: studentData })
    });
}
