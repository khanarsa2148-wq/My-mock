const API_URL = "https://script.google.com/macros/s/AKfycbyREqsJRy3heA8vgfAx2RcCrFfdzYUWy52DLg7SN83OcHM6PqLe4bkJdQZYHI-RnR8HwQ/exec";
...
async function validateUser() {
    ...
    // We use a GET request for authorization to bypass most mobile browser CORS blocks
    const authUrl = `${API_URL}?action=checkAuth&email=${encodeURIComponent(email)}`;
    ...
    try {
        ...
        // Fallback for some browsers that block the JSON response but allow the script to run
        console.log("Auth Error:", e);
        showModal("Connection Note", "Please ensure your internet is stable and try again.");
        btn.disabled = false; btn.innerText = "START EXAMINATION";
    }
}
...
async function endExam() {
    ...
    // Submitting scores via a hidden form post to avoid CORS failures on end of exam
